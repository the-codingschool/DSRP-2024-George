...
data <- data[-to_drop, ] #dropping those indexes
```
...
coef_plot_df <- data.frame(Features = names, coefs=feature_coefs)

#the plot of coefficients
ggplot(coef_plot_df, mapping=aes(Features, coefs)) + geom_point() + ylim(-1,1) + geom_hline(yintercept=0, linetype="dashed") + labs(title="The Coefficients of the Features in the Logistic Regression Model", x="Features", y="Coefficients")
```

```{r}
#the plot of the odd ratios
...
or_plot_df <- data.frame(
  term = names,
  estimate = odds_ratios[-1],
  conf.low = conf_int[, 1][-1],
  conf.high = conf_int[, 2][-1]
)

# Plot odds ratios using ggplot2
ggplot(or_plot_df, aes(x = term, y = estimate)) +
  geom_point() +
...
-   Life Expectancy is a big predictor of a country's self-sustainability

-   Despite having low correlation with self sustainability, population does seem to significigantly hurt a country's self-sustainability

-   There are a couple of features that seem to positively correlate with self-sustainability (not shown by correlation), however the correlation is quite weak
