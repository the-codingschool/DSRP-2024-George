...
```{r}
#sdgi vs. eco diff
ggplot(other_and_sustainable_data, mapping=aes(x=sdgi, y=eco_difference, color=region)) + geom_point() + ylim(-15, 15) + labs(title="SDGI vs. Ecological Difference", x="SDGI", y="Eco Difference") + geom_hline(yintercept=0, linetype="dashed")

#hdi vs. eco diff
ggplot(other_and_sustainable_data, mapping=aes(x=hdi, y=eco_difference, color=region)) + geom_point() + ylim(-15, 15) + labs(title="HDI vs. Ecological Difference", x="HDI", y="Eco Difference") + geom_hline(yintercept=0, linetype="dashed")

#life_expectancy vs. eco diff
ggplot(other_and_sustainable_data, mapping=aes(x=life_expectancy, y=eco_difference, color=region)) + geom_point() + ylim(-15, 15) + labs(title="Life Expectancy vs. Ecological Difference", x="Life Expectancy", y="Eco Difference") + geom_hline(yintercept=0, linetype="dashed")
```

```{r}
#gdp vs. eco diff
ggplot(other_and_sustainable_data, mapping=aes(x=per_capita_gdp, y=eco_difference, color=region)) + geom_point() + ylim(-15, 15) + labs(title="GDP Per Capita vs. Ecological Difference", x="GDP Per Capita ($)", y="Eco Difference") + scale_x_log10(breaks=c(1e+03, 1e+04, 1e+05), labels=c("1000", "10,000", "100,000")) + geom_hline(yintercept=0, linetype="dashed")
```

Looking at Correlation and Variance
...
```{r}
#save that data as an R object
#saveRDS(other_and_sustainable_data, file="AayanProject/ecoData.rds")
#commented because it has already been saved
```
